...
// Input validation schema - limit audio to ~7MB base64 encoded
const voiceSchema = z.object({
  audio: z.string().max(10000000, 'Audio data too large. Maximum size is approximately 7MB.'),
});
...
serve(async (req) => {
  ...
  try {
    ...
    const parseResult = voiceSchema.safeParse(rawInput);
